...
class View(Tk):
    ...
    def create_two_frames(self):
        ...
        return frame_top, frame_bottom, frame_img  # method return three objects (meetod tagastab kaks objekti)

    def create_all_buttons(self):
        # New Game
        btn_new = Button(self.frame_top, text='Uus mäng', font=self.default_style,
                         command=self.controller.click_btn_new)
        Button(self.frame_top, text='Edetabel', font=self.default_style,
               command=self.controller.click_btn_leaderboard).grid(row=0, column=1, padx=5, pady=2, sticky=EW)
        # Cancel and Send
        btn_cancel = Button(self.frame_top, text='Katkesta', font=self.default_style, state='disabled',
                            command=self.controller.click_btn_cancel)
        btn_send = Button(self.frame_top, text='Saada', font=self.default_style, state='disabled',
    ...
    def create_all_labels(self):
        Label(self.frame_top, text='Sisesta täht', font=self.default_style_bold).grid(row=1, column=0, padx=5,
                                                                                         pady=2)
        lbl_error = Label(self.frame_top, text='Valesid tähti 0', anchor='w', font=self.default_style_bold)
        lbl_time = Label(self.frame_top, text='0:00:00', font=self.default_style)
        lbl_result = Label(self.frame_bottom, text='Mängime'.upper(), font=self.big_font_style)
        # Image Label
    ...
    def generate_leaderboard(frame, data):
        ...
        my_table.heading('misses', text='Valed tähed', anchor=CENTER)
        my_table.heading('game_time', text='Aeg', anchor=CENTER)

        # Add data into table
        x = 0
        for p in data:
            # from file format (%Y-%m-%d %T) to
